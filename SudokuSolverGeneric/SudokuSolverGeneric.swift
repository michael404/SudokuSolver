extension SudokuBoardGeneric {
    
    func findFirstSolution() throws -> SudokuBoardGeneric {
        try findFirstSolution(using: Xoroshiro())
    }
    
    func findFirstSolution<R: RNG>(using rng: R) throws -> SudokuBoardGeneric {
        var solver = try SudokuSolverGeneric(eliminating: self, rng: rng)
        return try solver.guessAndEliminate(transformation: NormalGeneric<SudokuType>.self)
    }
    
    static func randomFullyFilledBoard() -> SudokuBoardGeneric {
        return randomFullyFilledBoard(using: Xoroshiro())
    }
    
    static func randomFullyFilledBoard<R: RNG>(using rng: R) -> SudokuBoardGeneric {
        var solver = try! SudokuSolverGeneric(eliminating: SudokuBoardGeneric.empty, rng: rng)
        return try! solver.guessAndEliminate(transformation: ShuffleGeneric<SudokuType>.self)
    }
    ...
    func numberOfSolutions<R: RNG>(using rng: R) -> NumberOfSolutions {
        do {
            ...
            let firstSolution = try solver1.guessAndEliminate(transformation: NormalGeneric<SudokuType>.self)
            let lastSolution = try solver2.guessAndEliminate(transformation: ReverseGeneric<SudokuType>.self)
...
struct SudokuSolverGeneric<SudokuType: SudokuTypeProtocol, R: RNG> {
    ...
    typealias Board = SudokuBoardGeneric<SudokuType>
