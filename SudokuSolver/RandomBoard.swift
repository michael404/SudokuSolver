...
internal extension SudokuBoard {
    ...
    func minimizingCluesParallel<R: RNG>(using rng: inout R, nodeLimit: Int = .max) -> SudokuBoard {
        ...
        let resultQueue = DispatchQueue(label: "SudokuSolver.minimizingCluesParallel")
        var screenedRemovable: Set<Int> = []
        DispatchQueue.concurrentPerform(iterations: solvedIndices.count) { position in
            ...
            if removable {
                resultQueue.sync { _ = screenedRemovable.insert(index) }
            }
        }
